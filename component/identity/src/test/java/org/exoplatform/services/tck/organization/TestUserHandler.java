...
public class TestUserHandler extends AbstractOrganizationServiceTest
{
   ...
   {
      assertTrue(uHandler.getUserPageList(10).getAll().size() >= 4);
   }
   ...
   public void testFindAllUsers() throws Exception
   {
      ...
      int initSize = uHandler.findAllUsers().getSize();
      assertTrue(initSize >= 5);
      assertSizeEquals(initSize, uHandler.findAllUsers(UserStatus.ENABLED), UserStatus.ENABLED);
      assertSizeEquals(initSize, uHandler.findAllUsers(UserStatus.ANY), UserStatus.ANY);
      ...
      boolean unsupportedOperation = false;
      int initialEnabledSize = uHandler.findAllUsers(UserStatus.ENABLED).getSize();
      int initialDisabledSize = uHandler.findAllUsers(UserStatus.DISABLED).getSize();
      try
      {
         // Disable the user
         uHandler.setEnabled(userName, false, true);

         assertSizeEquals(initialEnabledSize - 1, uHandler.findAllUsers(), UserStatus.ENABLED);
         assertSizeEquals(initialEnabledSize - 1, uHandler.findAllUsers(UserStatus.ENABLED), UserStatus.ENABLED);
         assertSizeEquals(initialDisabledSize + 1, uHandler.findAllUsers(UserStatus.DISABLED), UserStatus.DISABLED);
         assertSizeEquals(initialDisabledSize + initialEnabledSize, uHandler.findAllUsers(UserStatus.ANY), UserStatus.ANY);

         // Enable the user
         uHandler.setEnabled(userName, true, true);

         assertSizeEquals(initialEnabledSize, uHandler.findAllUsers(), UserStatus.ENABLED);
         assertSizeEquals(initialEnabledSize, uHandler.findAllUsers(UserStatus.ENABLED), UserStatus.ENABLED);
         assertSizeEquals(initialDisabledSize, uHandler.findAllUsers(UserStatus.DISABLED), UserStatus.DISABLED);
         assertSizeEquals(initialDisabledSize + initialEnabledSize, uHandler.findAllUsers(UserStatus.ANY), UserStatus.ANY);
      }
      catch (UnsupportedOperationException e)
      {
      ...
      initialEnabledSize = uHandler.findAllUsers(UserStatus.ENABLED).getSize();

      assertSizeEquals(initialEnabledSize, uHandler.findAllUsers(), UserStatus.ENABLED);
      assertSizeEquals(initialEnabledSize, uHandler.findAllUsers(UserStatus.ENABLED), UserStatus.ENABLED);
      assertSizeEquals(initialEnabledSize, uHandler.findAllUsers(UserStatus.ANY), UserStatus.ANY);
