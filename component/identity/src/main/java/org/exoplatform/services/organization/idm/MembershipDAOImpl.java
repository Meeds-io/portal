...
public class MembershipDAOImpl extends AbstractDAOImpl implements MembershipHandler {
  ...
  public void linkMembership(User user, Group g, MembershipType mt, boolean broadcast) throws Exception {
    if (log.isTraceEnabled()) {
      Tools.logMethodIn(log,
                        LogLevel.TRACE,
                        "linkMembership",
                        new Object[] { "user", user, "group", g, "membershipType",
    ...
    if (g1 == null) {
      throw new InvalidNameException("Can not create membership record for " + user.getUserName() + " because group " +
          g.getGroupName() + " is not exist");
    }
    ...
    String groupId = getIdentitySession().getPersistenceManager()
          ...
          try {
            if (getIdentitySession().getRelationshipManager().isAssociatedByKeys(groupId, m.getUserName())) {
              getIdentitySession().getRelationshipManager()
                                  .disassociateUsersByKeys(groupId, new ArrayList<String>(Arrays.asList(m.getUserName())));
            }
          } catch (IdentityException e1) {
  ...
  public Membership removeMembership(String id, boolean broadcast) throws Exception {
    ...
    try {
      ...
      // If MembershipID is not valid with format
  ...
  public Collection removeMembershipByUser(String userName, boolean broadcast) throws Exception {
    ...
    if (log.isTraceEnabled()) {
      Tools.logMethodIn(log,
                        ...
                        new Object[] { "userName", userName, "broadcast",
                          broadcast });
    }
  ...
  public Membership findMembershipByUserGroupAndType(String userName, String groupId, String type) throws Exception {
    ...
    if (role != null
        && (!isAssociationMapped() || !getAssociationMapping().equals(role.getRoleType())
  ...
  public Collection findMembershipsByUserAndGroup(String userName, String groupId) throws Exception {
    ...
    if (userName == null) {
      // julien fix : if user name is null, need to check if we do need to
      // return a special group
  ...
  public Collection findMembershipsByUser(String userName, boolean isIncludeInherited) throws Exception {
    Collection<Membership> memberships = findMembershipsByUser(userName);
    if (isIncludeInherited) {
      Collection<Membership> result = new LinkedList<>(memberships);
      List<MembershipImpl> inheritedMemberships = getInheritedMemberships(memberships);
      result.addAll(inheritedMemberships);
      return result;
    } else {
      return memberships;
    }
  ...
  public ListAccess<Membership> findAllMembershipsByUser(User user) throws Exception {
    org.picketlink.idm.api.User gtnUser = service_.getIdentitySession()
  ...
  protected List<MembershipImpl> getInheritedMemberships(Collection<Membership> memberships) {
    return memberships.stream()
                      .map(this::getInheritedMemberships)
                      .filter(CollectionUtils::isNotEmpty)
                      .flatMap(Set::stream)
                      .filter(m -> !memberships.contains(m))
                      .toList();
  }
  ...
  @SneakyThrows
  private Set<MembershipImpl> getInheritedMemberships(Membership m) {
    Group group = orgService.getGroupHandler().findGroupById(m.getGroupId());
    if (CollectionUtils.isEmpty(group.getEnclosingMemberships())) {
      return Collections.emptySet();
    } else {
      return group.getEnclosingMemberships()
                  .stream()
                  .map(im -> getInheritedMemberships(im, m))
                  .flatMap(Set::stream)
  ...
  @SneakyThrows
  private Set<MembershipImpl> getInheritedMemberships(NestedMembership nestedMembership, Membership membership) {
    Group parentGroup = orgService.getGroupHandler().findGroupById(nestedMembership.getGroupId());
    if (parentGroup != null
        && (nestedMembership.isIncludeAllMembershipTypes()
            || membership.getMembershipType().equals(NestedMembership.ALL_MEMBERSHIP_TYPE)
            || nestedMembership.getNestedMembershipType().equals(membership.getMembershipType()))) {
      Set<MembershipImpl> memberships = new HashSet<>();
      MembershipImpl inheritedMembership;
      if (nestedMembership.isInheritMembershipType()) {
        inheritedMembership = new MembershipImpl(membership.getMembershipType(),
                                                 membership.getUserName(),
                                                 nestedMembership.getGroupId(),
                                                 true);
      } else {
        inheritedMembership = new MembershipImpl(nestedMembership.getMembershipType(),
                                                 membership.getUserName(),
                                                 nestedMembership.getGroupId(),
                                                 true);
      }
      memberships.add(inheritedMembership);
      memberships.addAll(getInheritedMemberships(inheritedMembership));
      return memberships;
    } else {
      return Collections.emptySet();
    }
