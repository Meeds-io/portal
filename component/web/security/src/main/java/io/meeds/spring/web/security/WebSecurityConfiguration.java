...
public class WebSecurityConfiguration implements ServletContextAware {
  ...
  public SecurityFilterChain filterChain(HttpSecurity http,
    ...
    return this.customizeHttpSecurity(http)
               ...
               // Bridge container-managed (JAAS/Tomcat) authentication into
               // Spring Security's SecurityContext so that both the Servlet API
               // (getRemoteUser, getUserPrincipal, isUserInRole) and Spring
               // Security APIs (@Secured, SecurityContextHolder,
               // @AuthenticationPrincipal, ...)
               // expose the same authenticated user.
