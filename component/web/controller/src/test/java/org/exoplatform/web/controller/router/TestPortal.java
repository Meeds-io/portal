...
public class TestPortal extends AbstractTestController {
    ...
    public void testLanguage1() throws Exception {
        Router router = router().add(
                route("/public/{gtn:lang}").with(pathParam("gtn:lang").matchedBy(LANG_PATTERN).preservePath())).build();
    ...
    public void testLanguage2() throws Exception {
        Router router = router().add(route("/{gtn:lang}/public").with(pathParam("gtn:lang").matchedBy(LANG_PATTERN))).build();
    ...
    public void testLanguage3() throws Exception {
        Router router = router().add(
                route("/public/{gtn:lang}/{gtn:sitename}{gtn:path}").with(
                        pathParam("gtn:lang").matchedBy(LANG_PATTERN).preservePath()).with(
    ...
    public void testDuplicateRouteWithDifferentRouteParam() throws Exception {
        Router router = router().add(
                route("/").with(routeParam("foo").withValue("foo_1")).with(
                        requestParam("bar").named("bar").matchedByLiteral("bar_value")),
                route("/").with(routeParam("foo").withValue("foo_2"))).build();
    ...
    public void testJSMin() throws Exception {
        Router router = router().add(
                route("/foo{gtn:min}.js").with(pathParam("gtn:min").matchedBy("-(min)|").captureGroup(true))).build();
