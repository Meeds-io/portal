...
public class InMemorySettingScopeDAO extends AbstractInMemoryDAO<ScopeEntity> implements SettingScopeDAO {

  @Override
  public ScopeEntity getScopeByTypeAndName(String scopeType, String scopeName) {
    return entities.values()
                   .stream()
                   .filter(entity -> (StringUtils.isBlank(scopeName) || StringUtils.equals(scopeName, entity.getName()))
                       && (StringUtils.isBlank(scopeType) || StringUtils.equals(scopeType, entity.getType())))
                   .findFirst()
                   .orElse(null);
  }
