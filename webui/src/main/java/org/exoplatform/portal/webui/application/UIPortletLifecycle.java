...
public class UIPortletLifecycle extends Lifecycle<UIPortlet> {
   ...
   * following work: 1) If the current request is one that target the portal
   * than an event targeting a Portal level ActionListener is sent. This case
   * happen when the incoming request contains the parameter
   ...
   */
  public void processAction(UIPortlet uicomponent, WebuiRequestContext context) throws Exception { // NOSONAR
    try {
      // The PortletMode and WindowState can change during a portlet invocation,
      // so we need
      // to be able to compare the results before and after invoking the portlet
      // to know if
       ...
       * RenderURL one In case of a RenderURL, the parameter state map must be
       * invalidated and this is done in the associated ActionListener If no
       * action type is specified we assume the default, which is to render
       */
      ...
      // These two checks needs to go after the ProcessAction, ServeResource or
      // Render broadcast events.
      // The mode or state can change during the invocation and we need to be
      // able to broadcast the change
      ...
      if (currentWindowState != null && !currentWindowState.equals(uicomponent.getCurrentWindowState())) {
        context.setAttribute(UIPortletActionListener.CHANGE_WINDOW_STATE_EVENT,
                             uicomponent.getCurrentWindowState()
                                        .toString());
        Event<UIComponent> event = uicomponent.createEvent("ChangeWindowState", Event.Phase.PROCESS, context);
        if (event != null)
          event.broadcast();
   ...
   * the portlet 1) Create a RenderInput object and fill it with all the Request
   * information 2) Call the portletContainer.render() method of the Portlet
   * Container to get the HTML generated fragment 3) Then if the current request
   * is an AJAX one, just write in the buffer the content returned by the
   * portlet container 4) If not AJAX, then merge the content with the
   * UIPortlet.gtmpl
   */
