...
public class UIPortletActionListener {
   ...
   * portlet container has been invoked by the client <br>
   ...
   * page
   */
  public static class ProcessActionActionListener<I> extends EventListener<UIPortlet> {
    public void execute(Event<UIPortlet> event) throws Exception {
      ...
      if (portletResponse instanceof UpdateNavigationalStateResponse updateNavigationalStateResponse) {
        handleUpdateNavigationalStateResponse(updateNavigationalStateResponse, uiPortlet, prcontext);
      } else if (portletResponse instanceof HTTPRedirectionResponse httpRedirectionResponse) {
        handleRedirectionResponse(httpRedirectionResponse, prcontext.getResponse());
        prcontext.setResponseComplete(true);
      } else if (portletResponse instanceof ErrorResponse errorResponse) {
        handleErrorResponse(errorResponse);
      } else if (portletResponse instanceof SecurityResponse securityResponse) {
        handleSecurityResponse(securityResponse);
      } else {
        throw new IllegalStateException(String.format("Unexpected response type [%s]. Expected an UpdateNavigationResponse , a HTTPRedirectionResponse or an ErrorResponse.",
                                                      portletResponse));
      }
    }
    ...
    private void handleUpdateNavigationalStateResponse(UpdateNavigationalStateResponse navStateResponse,
                                                       UIPortlet uiPortlet,
                                                       PortalRequestContext prcontext) throws Exception {
      /*
       * Update the portlet window state according to the action output
       * information If the current node is displaying a usual layout page, also
       * tells the page which portlet to render or not when the state is
       * maximized
       */
      // Note: we should only update the WindowState if the
      // UpdateNavigationalStateResponse.getWindowState is not null,
      // otherwise it means the WindowState has not changed and we should use
      // the current value.
      ...
      // otherwise it means the mode has not changed and we should use the
      // current value.
      ...
      // update the public render parameters with the changes from the
      // invocation
      ...
      // TODO: (mwringe) add this to the UpdateNavigationStateResponse.Event
  ...
  /**
   * This method is used to set the next portlet window state if this one needs
   * to be modified because of the incoming request
   */
  ...
  /**
   * This method is used to set the next portlet mode if this one needs to be
   * modified because of the incoming request
   */
   ...
   * previous spec - JSR 168 - a servlet was needed) - provide text output that
   * does not impact the entire portal rendering, it is for instance usefull
   * when dealing with Javascript to return some JSON structures <br>
   * The method delegates the call to the portlet container serverResource
   * method after filling the ResourceInput object with the current request
   * state. <br>
   * This returns a ResourceOutput object that can content binary or text
   * contentType <br>
   * Finally the content is set in the portal response writer or outputstream
   * depending on the type; the processRender() method of the portal is not
   * called as we set the response as complete
   */
    ...
    public void execute(Event<UIPortlet> event) throws Exception {
      ...
      try {
        ...
        if (!(portletResponse instanceof ContentResponse)) {
          ...
          //
          log.trace("Try to get a resource of type: " + contentType + " for the portlet: " + uiPortlet.getId());
  ...
  public static class ProcessEventsActionListener extends EventListener<UIPortlet> {
    public void execute(Event<UIPortlet> event) throws Exception {
      ...
      /*
       * Iterate over all the events that the processAction has generated. Check
       * among all the portlet instances deployed in the page (usual layout or
       * webos) which instance can be targeted by the event and then process the
       * event on the associated UIPortlet component
       */
      while (events.size() > 0) {
   ...
   * current portlet stored in the Portlet Caontainer <br>
   ...
   */
  public static <I> List<javax.portlet.Event> processEvent(UIPortlet uiPortlet, javax.portlet.Event event) {
    log.trace("Process Event: " + event.getName() + " for portlet: " + uiPortlet.getState());
    PortalRequestContext context = PortalRequestContext.getCurrentInstance();

    //
    PortletInvocationResponse piResponse;
    try {
      ...
      piResponse = uiPortlet.invoke(eventInvocation);
    } catch (Exception e) {
      throw new IllegalStateException("Problem while processesing event for the portlet: " + uiPortlet.getState(), e);
    }

    if (piResponse instanceof UpdateNavigationalStateResponse navResponse) {
      /*
       * Update the portlet window state according to the action output
       * information If the current node is displaying a usual layout page, also
       * tells the page which portlet to render or not when the state is
      ...
      if (piResponse instanceof ErrorResponse errorResponse) {
        throw new IllegalStateException(String.format("Unexpected response type [%s]. Expected a UpdateNavigationResponse or an ErrorResponse.",
                                                      piResponse),
  ...
  public static class RenderActionListener extends EventListener<UIPortlet> {
    public void execute(Event<UIPortlet> event) throws Exception {
      ...
      // set render params
      String navState = event.getRequestContext()
                             .getRequestParameter(
   ...
   * aka during the processDecode() phase of our UI framework <br>
  ...
  /**
   * This listener is called when the portlet portlet window state has to be
   * changed. It can be changed by building specific URL, or by programatically
   * in the portlet, then triggered after action or event response
   */
