...
public class Lifecycle<E extends UIComponent> {
    ...
    /**
     * That method is the most generic one for every UIComponent that is bound to this Lifecycle object and the class that
     * extends it without overriding the method.
     *
     * The template associated to the specified UIComponent is rendered using renderTemplate(). A WebuiBindingContext context
     * object provides the template with all the necessary objects to render. (WebuiBindingContext extends the Map class)
     *
     */
     ...
     * <li>If the writer used to render the output is of type HtmlValidator, which is the case in the Portal environment, then
     * it is also possible to validate the generated HTML (for debugging purposes)</li>
